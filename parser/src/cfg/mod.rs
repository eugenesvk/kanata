...
#[cfg(all(not(feature="interception_driver"),any(not(feature="win_llhook_read_scancodes"),not(feature="win_sendinput_send_scancodes")),target_os = "windows"))]
const DEF_LOCAL_KEYS: &str = "deflocalkeys-win";
#[cfg(all(feature = "win_llhook_read_scancodes",feature = "win_sendinput_send_scancodes",target_os = "windows"))]
...
// A snapshot of enviroment variables, or an error message with an explanation why env vars are not not supported.
...
fn parse_cfg_raw(p: &Path, s: &mut ParserState) -> MResult<IntermediateCfg> {
  ...
  parse_cfg_raw_string(&text,s,p,&mut file_content_provider,DEF_LOCAL_KEYS,env_vars,).map_err(|e| e.into())
}
...
pub fn parse_cfg_raw_string(
  ...
  for def_local_keys_variant in DEFLOCALKEYS_VARIANTS {
    if let Some((result, span)) = spanned_root_exprs.iter()
      .find(gen_first_atom_filter_spanned(def_local_keys_variant))
      .map(|x| {(parse_deflocalkeys(def_local_keys_variant, &x.t),x.span.clone(),)})
      {
        let mapping = result?;
        if def_local_keys_variant == &def_local_keys_variant_to_apply {
          assert!(local_keys.is_none(),">1 mutually exclusive deflocalkeys variants were parsed");
          local_keys = Some(mapping);
        } else {
          lsp_hint_inactive_code.push(LspHintInactiveCode {
            span,reason: format!("Another localkeys variant is currently active: {def_local_keys_variant_to_apply}"),})        }
    }
  ...
  let deflayer_spanned_filter = |exprs: &&Spanned<Vec<SExpr>>| -> bool {
    if exprs.t.is_empty()                 	{return false;}
    if let SExpr::Atom(atom) = &exprs.t[0]	{deflayer_labels.contains(&atom.t.as_str())
    } else                                	{false}
  };

  let layer_exprs = spanned_root_exprs.iter().filter(deflayer_spanned_filter).cloned()
    .map(|e| match e.t[0].atom(None).unwrap() {
      ...
      _              	=> unreachable!(),})
    .collect::<Vec<_>>();
  if layer_exprs.is_empty() {bail!("No deflayer expressions exist. At least one layer must be defined.")}
  ...
  let deflayer_filter = |exprs: &&Vec<SExpr>| -> bool {
    ...
    if let SExpr::Atom(atom) = &exprs[0]	{deflayer_labels.contains(&atom.t.as_str())
    } else                              	{false}
  };
  ...
  let layer_exprs = root_exprs.iter().filter(deflayer_filter).cloned()
  ...
  *s = ParserState {
  ...
  let chords_v2 = match chords_v2_exprs.len() {
    ...
    _ => {
      let spanned = spanned_root_exprs.iter().filter(gen_first_atom_filter_spanned("defchordsv2-experimental")).nth(1).expect("> 2 overrides");
      bail_span!(spanned,"Only one defchordsv2 allowed, found more.\nDelete the extras.")    }
  };
  s.trans_forbidden_reason = None;
  if chords_v2.is_some() && !cfg.concurrent_tap_hold {
    return Err(anyhow!("With defchordsv2 defined, concurrent-tap-hold in defcfg must be true.\nIt is currently false or unspecified.").into());
  }
...
fn parse_layer_indexes(exprs: &[SpannedLayerExprs], expected_len: usize) -> Result<LayerIndexes> {
  ...
  for (i, expr_type) in exprs.iter().enumerate() {
    ...
    let layer_name = match expr_type {
      SpannedLayerExprs::DefsrcMapping(_) => layer_expr.atom(None).ok_or_else(|| {anyhow_expr!(layer_expr, "layer name after {DEFLAYER} must be a string")})?.to_owned(),
      SpannedLayerExprs::CustomMapping(_) => {
        let list = layer_expr.list(None).ok_or_else(|| {anyhow_expr!(layer_expr,"layer name after {DEFLAYER_MAPPED} must be in parentheses")})?.to_owned();
        if list.len() != 1 {bail_expr!(layer_expr,"layer name after {DEFLAYER_MAPPED} must be a string within one pair of parentheses");}
    ...
    if layer_indexes.get(&layer_name).is_some() {bail_expr!(layer_expr, "duplicate layer name: {}", layer_name);}
    for subexpr in subexprs { // Check if user tried to use parentheses directly - `(` and `)` or escaped them like in kmonad - `\(` and `\)`.
      if let Some(list) = subexpr.list(None) {
        if list.is_empty() {bail_expr!(subexpr,"You can't put parentheses in deflayer directly, because they are special characters for delimiting lists.\nTo get `(` and `)` in US layout, you should use `S-9` and `S-0` respectively.\nFor more context, see: https://github.com/jtroo/kanata/issues/459")}
        if list.len() == 1
          && list.first().is_some_and(|s| s.atom(None).is_some_and(|atom| atom == "\\")) {
          bail_expr!(subexpr,"Escaping shifted characters with `\\` is currently not supported in kanata.\nTo get `(` and `)` in US layout, you should use `S-9` and `S-0` respectively.\nFor more context, see: https://github.com/jtroo/kanata/issues/163")}
      }
    }
...
#[derive(Debug,Clone)] enum LayerExprs        {
  DefsrcMapping(Vec<SExpr>),
  CustomMapping(Vec<SExpr>),}
#[derive(Debug,Clone)] enum SpannedLayerExprs {
  DefsrcMapping(Spanned<Vec<SExpr>>),
  CustomMapping(Spanned<Vec<SExpr>>),}

#[derive(Debug)] pub struct ParserState {
...
fn parse_aliases(exprs: &[&Spanned<Vec<SExpr>>],s: &mut ParserState,env_vars: &EnvVars,) -> Result<()> {
  for expr in exprs {
...
fn handle_envcond_defalias(exprs:&Spanned<Vec<SExpr>>,s:&mut ParserState,env_vars:&EnvVars,) -> Result<()> {
...
fn custom(ca:CustomAction, a:&Allocations) -> Result<&'static KanataAction> {Ok(a.sref(Action::Custom(a.sref(a.sref_slice(ca)))))}
...
fn parse_action_atom(ac_span: &Spanned<String>, s: &ParserState) -> Result<&'static KanataAction> {
  ...
  match ac {
    "_"|"‗"|"≝"                	=> {if let Some(trans_forbidden_reason) = s.trans_forbidden_reason {bail_span!(ac_span, "{trans_forbidden_reason}");
      } else                   	  {return Ok(s.a.sref(Action::Trans));}}
  ...
  if let Some(unisym) = ac.strip_prefix('🔣') {return custom(CustomAction::Unicode(unisym.chars().next().expect("1 char")),&s.a,);}

  // Parse a sequence like `C-S-v` or `C-A-del`
  let (mut keys, unparsed_str) = parse_mod_prefix(ac)?;
  keys.push(str_to_oscode(unparsed_str).ok_or_else(|| { // check aliases
    if s.aliases.contains_key(ac) {anyhow!("Unknown key/action: {ac}. If you meant to use an alias, prefix it with '@' symbol: @{ac}")
    } else if s.vars.contains_key(ac) {anyhow!("Unknown key/action: {ac}. If you meant to use a variable, prefix it with '$' symbol: ${ac}")
    } else {anyhow!("Unknown key/action: {ac}")}})?.into(),
  );
...
fn parse_action_list(ac: &[SExpr], s: &ParserState) -> Result<&'static KanataAction> {
  if ac.is_empty() {return Ok(s.a.sref(Action::NoOp));}
...
fn parse_push_message(ac_params: &[SExpr], s: &ParserState) -> Result<&'static KanataAction> {
  if ac_params.is_empty() {bail!("{PUSH_MESSAGE} expects at least one item, an item can be a list or an atom, found 0, none");}
...
fn to_simple_expr(params: &[SExpr], s: &ParserState) -> Vec<SimpleSExpr> {
  ...
  for param in params {
    ...
    } else { // unwrap: this must be a list, since it's not an atom.
...
fn parse_layers(s: &ParserState,mapped_keys: &mut MappedKeys,defcfg: &CfgOptions,) -> Result<IntermediateLayers> {
  ...
  for (layer_level, layer) in s.layer_exprs.iter().enumerate() {
    match layer { // The skip is done to skip the the `deflayer` and layer name tokens.
      LayerExprs::DefsrcMapping(layer) => {
        for (i, ac) in layer.iter().skip(2).enumerate() { // Parse actions in the layer and place them appropriately according to defsrc mapping order.
      ...
      LayerExprs::CustomMapping(layer) => { // Parse actions as input -> output triplets
        ...
        for triplet in pairs.by_ref() {
          ...
          if input.atom(s.vars()).is_some_and(|x| x == "_") {
              ...
              if !defcfg.process_unmapped_keys {bail_expr!(input,"must set process-unmapped-keys to yes to use __ to map unmapped keys");}
              if both_anykey_used {bail_expr!(input, "must either use __ or ___ within a layer, not both")}
              ...
              if both_anykey_used    	{bail_expr!(input, "must have only one use of ___ within a layer")}
              if defsrc_anykey_used  	{bail_expr!(input, "must either use _ or ___ within a layer, not both")}
              if unmapped_anykey_used	{bail_expr!(input, "must either use __ or ___ within a layer, not both")}
              if !defcfg.process_unmapped_keys {bail_expr!(input,"must set process-unmapped-keys to yes to use ___ to also map unmapped keys");}
              for i in 0..layers_cfg[0][0].len() {
                  if layers_cfg[layer_level * 2    ][0][i] == Action::Trans {
                     layers_cfg[layer_level * 2    ][0][i] = *action;
                     layers_cfg[layer_level * 2 + 1][0][i] = *action;
        ...
        match rem.len() {
          ...
          2 => {bail_expr!(&rem[1], "map string must be followed by an action");}
          _ => unreachable!(),
        }
      }
    }
    for (layer_action, defsrc_action) in layers_cfg[layer_level * 2][0].iter_mut().zip(defsrc_layer) {
      if *layer_action == Action::Trans { // Set transparent actions in the "layer-switch" version of the layer according to defsrc action.
        if s.block_unmapped_keys	{*layer_action = Action::NoOp;
        } else                  	{*layer_action = defsrc_action;}
      }
    }
...
fn create_key_outputs(layers:&KanataLayers,overrides:&Overrides,chords_v2:&Option<ChordsV2<'static, KanataCustom>>,) -> KeyOutputs {
  ...
  for (layer_idx, layer) in layers.iter().enumerate() {
    ...
    for (i, action) in layer[0].iter().enumerate() {
      ...
      add_chordsv2_output_for_key_pos      (osc_slot, layer_idx, chords_v2, &mut layer_outputs, overrides,);
    }
...
fn add_chordsv2_output_for_key_pos(
  ...
  let Some(chords_v2) = chords_v2.as_ref() else {return;};
  let Some(chords_for_key) = chords_v2.chords().mapping.get(&u16::from(osc_slot)) else {return;};
  for chord in chords_for_key.chords.iter() {
    if !chord.disabled_layers.contains(&(layer_idx as u16)) {add_key_output_from_action_to_key_pos(osc_slot, chord.action, outputs, overrides);}  }
}
