...
fn parse_action_list(ac: &[SExpr], s: &ParserState) -> Result<&'static KanataAction> {
    ...
    match ac_type.as_str() {
        ...
        MACRO_CANCEL_ON_NEXT_PRESS => {parse_macro_cancel_on_next_press(&ac[1..], s, RepeatMacro::No)}
        MACRO_REPEAT_CANCEL_ON_NEXT_PRESS => {parse_macro_cancel_on_next_press(&ac[1..], s, RepeatMacro::Yes)}
        MACRO_CANCEL_ON_NEXT_PRESS_CANCEL_ON_RELEASE => {parse_macro_cancel_on_next_press_cancel_on_release(&ac[1..], s, RepeatMacro::No)}
        MACRO_REPEAT_CANCEL_ON_NEXT_PRESS_CANCEL_ON_RELEASE => {parse_macro_cancel_on_next_press_cancel_on_release(&ac[1..], s, RepeatMacro::Yes)}
...
enum CmdType {
    ...
    ClipboardSaveSet,	// Execute command and set clipboard save id to output. Clipboard save id content is passed as stdin to the command.
}
...
fn parse_cmd(
    ...
    #[cfg(not(feature = "cmd"))]{
        bail!("cmd is not enabled for this kanata executable. Use a cmd_allowed prebuilt executable or compile with the feature: cmd.");}
    #[cfg(    feature = "cmd")]{
        ...
        if !s.is_cmd_enabled {bail!("To use cmd you must put in defcfg: danger-enable-cmd yes.");}
        let mut cmd = vec![];
        collect_strings(ac_params, &mut cmd, s);
        if cmd.is_empty() {bail!(ERR_STR);}
        Ok(s.a
