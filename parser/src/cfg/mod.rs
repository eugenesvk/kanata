        ...
        let layer_name = match expr_type {
            ...
            SpannedLayerExprs::CustomMapping(_) => {
                let list = layer_expr
                    ...
                    .to_owned();
                if list.len() != 1 {
                    bail_expr!(layer_expr,"layer name after {DEFLAYER_MAPPED} must be a string within one pair of parentheses");
                }
    ...
    for (layer_level, layer) in s.layer_exprs.iter().enumerate() {
        match layer {
            ...
            LayerExprs::DefsrcMapping(layer) => {
                for (i, ac) in layer.iter().skip(2).enumerate() {
                    // Parse actions in the layer and place them appropriately according to defsrc mapping order.
            ...
            layers_cfg[layer_level * 2][0].iter_mut().zip(defsrc_layer)
        {
            if *layer_action == Action::Trans {
                // Set transparent actions in the "layer-switch" version of the layer according to defsrc action.
