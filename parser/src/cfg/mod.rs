    ...
    match layer { // The skip is done to skip the the `deflayer` and layer name tokens.
      ...
      LayerExprs::CustomMapping(layer) => { // Parse actions as input -> output triplets
        ...
        for triplet in triplets.by_ref() {
          ...
          let input_key = input.atom(s.vars()).and_then(str_to_oscode).ok_or_else(|| anyhow_expr!(input, "input must be a key name"))?;
          mapped_keys.insert(input_key);
          if !layer_mapped_keys.insert(input_key) {bail_expr!(input, "input key must not be repeated within a layer")}
          let mapstrs = ["=", ":", "->", ">>", "maps-to", "→", "🞂"];
