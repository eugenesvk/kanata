...
pub struct Kanata {
  ...
  pub override_states               	: OverrideStates                     	,// Reusable allocations to help with computing whether overrides are active based on key outputs.
      last_tick                     	: time::Instant                      	,// Time of the last tick to know how many tick iterations to run, to achieve a 1ms tick interval more closely.
  ...
  pub fake_keys                     	: HashMap<String, usize>             	,// Names of fake keys mapped to their index in the fake keys row
...
impl Kanata {
  ...
  pub fn new(args: &ValidatedArgs) -> Result<Self> {
    ...
    #[cfg(target_os="windows")] set_win_altgr_behaviour(cfg.options.windows_altgr);
    *MAPPED_KEYS.lock() = cfg.mapped_keys;
    Ok(Self {
      ...
      #[cfg(    target_os="macos")] include_names              	: cfg.options.macos_dev_names_include,
      #[cfg(    target_os="linux")] kbd_in_paths               	: cfg.options.linux_dev,
      #[cfg(    target_os="linux")] continue_if_no_devices     	: cfg.options.linux_continue_if_no_devs_found,
      #[cfg(    target_os="linux")] include_names              	: cfg.options.linux_dev_names_include,
      #[cfg(    target_os="linux")] exclude_names              	: cfg.options.linux_dev_names_exclude,
      #[cfg(all(target_os="windows"                            	//
        ,feature="interception_driver"))] intercept_mouse_hwids	: cfg.options.windows_interception_mouse_hwids,
      #[cfg(all(target_os="windows"                            	//
        ,feature="interception_driver"))] intercept_kb_hwids   	: cfg.options.windows_interception_keyboard_hwids,
      ...
      dynamic_macro_replay_behaviour                           	: ReplayBehaviour {delay: cfg.options.dynamic_macro_replay_delay_behaviour,},
      #[cfg(target_os="linux")] x11_repeat_rate                	: cfg.options.linux_x11_repeat_delay_rate,
      ...
      #[cfg(feature="tcp_server")] fake_keys                   	: cfg.fake_keys,
  ...
  fn do_live_reload(&mut self, _tx: &Option<Sender<ServerMessage>>) -> Result<()> {
    ...
    self.dynamic_macro_replay_behaviour = ReplayBehaviour {delay: cfg.options.dynamic_macro_replay_delay_behaviour,};
    #[cfg(feature="tcp_server")]{self.fake_keys = cfg.fake_keys;}
    ...
    let kbrn_ev = match event.value {
      KeyValue::Press   => {
        if let Some((macro_id, recorded_macro)) = record_press(
          &mut self.dynamic_macro_record_state, event.code,
               self.dynamic_macro_max_presses ,
        ) {self.dynamic_macros.insert(macro_id, recorded_macro);}
        Event::Press  (0, evc)}
      KeyValue::Release => {record_release(&mut self.dynamic_macro_record_state, event.code);
        Event::Release(0, evc)}
      KeyValue::Repeat  => {let ret = self.handle_repeat(event);return ret;}
      KeyValue::Tap     => {
  ...
  fn handle_keystate_changes(&mut self) -> Result<bool> {
    ...
    match custom_event { // Deal with unmodded. Unlike other custom actions, this should come before key presses and releases. I don't quite remember why custom actions come after the key processing, but I remember that it is intentional. However, since unmodded needs to modify the key lists, it should come before.
      CustomEvent::Press(custacts) => {
        for custact in custacts.iter() { match custact {
          CustomAction::Unmodded 	{keys}	=> {self.unmodded_keys .extend(keys);}
          CustomAction::Unshifted	{keys}	=> {self.unshifted_keys.extend(keys);}
          _                      	      	=> {}} }  }
      CustomEvent::Release(custacts) => {
        for custact in custacts.iter() { match custact {
          CustomAction::Unmodded 	{keys}	=> {self.unmodded_keys .retain(|k| !keys.contains(k));}
          CustomAction::Unshifted	{keys}	=> {self.unshifted_keys.retain(|k| !keys.contains(k));}
          _                      	      	=> {}} }  }
      _                              => {}
    }
    ...
    for k in cur_keys.iter() {
      if self.prev_keys.contains(k) {log::trace!("{k:?} is old press");
        continue;}
      ...
      match &mut self.sequence_state {
        ...
        Some(state) => {
          ...
          let pushed_into_seq = {
            ...
            for k in cur_keys.iter().copied() { base |= mod_mask_for_keycode(k); }
            base
          };

          state.sequence.push(pushed_into_seq);
          match state.sequence_input_mode {
            SequenceInputMode  ::VisibleBackspaced => {self.kbd_out.press_key(osc)?;}
          ...
          if res == NotInTrie {
            let is_invalid_termination = if self.sequence_backtrack_modcancel
              ...
              for i in (0..state.sequence.len()).rev() { // If applicable, check again with modifier bits unset.
            ...
            if is_invalid_termination {log::debug!("got invalid sequence; exiting sequence mode");
              match state.sequence_input_mode {
                SequenceInputMode::HiddenDelayType => {
          ...
          if let HasValue((i, j)) = res {
            ...
            for k in state.sequence.iter() {// Make sure to unpress any keys that were pressed as part of the sequence so that the keyberon internal sequence mechanism can do press+unpress of them.
...
fn check_for_exit(event: &KeyEvent) {
  ...
  if IS_ESC_PRESSED.load(SeqCst) && IS_SPC_PRESSED.load(SeqCst) && IS_LCL_PRESSED.load(SeqCst) {
    #[cfg(not(target_os="linux"))]{log::info!("{EXIT_MSG}"); panic!("{EXIT_MSG}");}
    #[cfg(    target_os="linux" )]{log::info!("{EXIT_MSG}"); signal_hook::low_level::raise(signal_hook::consts::SIGTERM).expect("raise signal");}
  }
}
