...
impl Kanata {
  ...
  pub fn new(args: &ValidatedArgs) -> Result<Self> {
    ...
    #[cfg(target_os="windows")] unsafe {// log::info!("Asking Windows to improve timer precision");
      if winapi::um::timeapi::timeBeginPeriod(1) == winapi::um::mmsystem::TIMERR_NOCANDO {bail!("failed to improve timer precision");}}
          ...
          Ok(event) => {
            ...
            for (id, client) in &mut *clients {
              match client.write_all(&notification) {
                Ok (_) => {log::debug!("layer change notification sent");}
                Err(e) => {log::warn!("removing tcp client where write failed: {id}, {e:?}");
                  stale_clients.push(id.clone());} // the client is no longer connected, let's remove them
              }
            }
      ...
      let err = loop {
        ...
        if can_block {
          ...
          match rx.recv() {
            Ok(kev) => {
              let mut k = kanata.lock();
              let now = instant::Instant::now().checked_sub(time::Duration::from_millis(1)).expect("−1ms from current time");
