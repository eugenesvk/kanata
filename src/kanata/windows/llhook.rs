...
impl Kanata {
    ...
    pub fn event_loop(_cfg: Arc<Mutex<Self>>, tx: Sender<KeyEvent>, #[cfg(all(target_os = "windows", feature = "gui"))] ui:crate::gui::system_tray_ui::SystemTrayUi,) -> Result<()> {
        ...
        let _kbhook = KeyboardHook::set_input_cb(move |input_event| {
            // This callback should return `false` if the input event is **not** handled by the callback and `true` if the input event **is** handled by the callback. Returning false informs the callback caller that the input event should be handed back to the OS for normal processing.
