...
pub struct KeyboardHook {handle:HHOOK}  // Wrapper for the low-level keyboard hook API. Automatically unregisters the hook when dropped.
impl KeyboardHook { /// Sets the low-level keyboard hook for this thread. Panics when a hook is already registered from the same thread.
  ...
  pub fn set_input_cb(callback: impl FnMut(InputEvent) -> bool + 'static) -> KeyboardHook {
    HOOK.with(|state| {assert!(state.take().is_none(),"Only one keyboard hook can be registered per thread.");
      state.set(Some(Box::new(callback)));
      KeyboardHook {handle:unsafe{
        SetWindowsHookExW(WH_KEYBOARD_LL, Some(hook_proc), ptr::null_mut(), 0).as_mut().expect("install low-level keyboard hook successfully")},}
...
impl InputEvent {
  ...
  fn from_hook_pinfo(pInfo:&KBDLLHOOKSTRUCT   ) -> Self {
    let code = if pInfo.vkCode == (           VK_RETURN as u32) {
      ...
      #[cfg(    feature = "win_llhook_read_scancodes" )]{ // The array below contains the lower bytes that can potentially have E1/E0 as an upper byte: learn.microsoft.com/en-us/windows/win32/inputdev/about-keyboard-input#scan-codes
        // If scancode is *potentially* extended it's probably not layout dependent right? Thus use vkCode, because MapVirtualKeyA does not do scanCode->vkCode correctly for extended keys.
        // There are some repeats so that we can have `(len % 16) == 0`. Contains operates on two usize`s of data at a time.`
        if [0x5E, 0x5F, 0x63, 0x37, 0x1D, 0x46, 0x52, 0x47, 0x49, 0x53, 0x4F, 0x51,
          ...
          ].contains(&((pInfo.scanCode & 0xff) as u8))
               {pInfo.vkCode
        } else {unsafe {MapVirtualKeyA(pInfo.scanCode,1)}} // learn.microsoft.com/en-us/windows/win32/api/winuser/nf-winuser-mapvirtualkeya
    }
  };
  Self {
    code,
...
#[cfg(not(feature="simulated_output"))] impl       KbdOut {
  ...
  pub fn write_code 	(&mut self, code : u32   , value:KeyValue	) -> Result<()  ,io::Error> {super::write_code(code as u16, value)}
...
fn move_mouse_xy	(x:i32, y:i32) {mouse_event(MOUSEEVENTF_MOVE                                                 	,0,x,y);}
fn set_mouse_xy 	(x:i32, y:i32) {mouse_event(MOUSEEVENTF_ABSOLUTE | MOUSEEVENTF_MOVE | MOUSEEVENTF_VIRTUALDESK	,0,x,y);}

fn mouse_event(flags: u32, data: u32, dx: i32, dy: i32) { // from Enigo: github.com/enigo-rs/enigo
  let mut input = INPUT {
    type_	: INPUT_MOUSE,
    u    	: unsafe {
