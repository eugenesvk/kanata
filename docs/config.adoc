...
[%collapsible]
====
* https://github.com/jtroo/kanata/blob/v1.6.0-prerelease-4/docs/config.adoc[v1.6.0-prerelease-4]
...
Show a custom tray icon file for a gui-enabled build of kanata on Windows.
Accepts either the full path (including the file name with extension) to the icon file
...
If not specified, ttries to load any icon file from the same locations with the name matching
...
Show a custom tray icon that matches the name of the active layer if it doesn't specify an explicit icon.
...
and its requires libraries (needs https://www.autohotkey.com/download/ahk-v2.exe[AutoHotkey v2] installed)
