...
pub extern "win64" fn input_ev_listener(vk: c_uint, sc: c_uint, up: c_int) -> LRESULT {
    ...
    if let Some(mut fnhook) = h_cbl.take() {
        // move our opt+boxed closure, replacing it with None, can't just .unwrap since Copy trait not implemented for dyn fnMut
        ...
        *h_cbl = Some(fnhook); // put our closure back
        ...
        error!("fnHook processing key events isn't available yet {key_event} from {vk} sc={sc} up={up}");
        0
    }
}
